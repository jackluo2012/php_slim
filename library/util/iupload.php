 ...
 */
 class IUpload
 {
    ...
    function __construct($size = 10000,$type = array())
    {
        ...
        $this->fdfs = IFdfs::getInstance(Config::get('fastDFS'));// IFdfs::getInstance();
    }
                ...
                foreach($keys as $key)
                {
                	...
                	else
                	{
		                ...
		                else
		                {
		                    ...
		                    {
                                $fileInfo[$key]['info'] = $this->moveToRemote($_FILES[$field]['tmp_name'][$key],$fileext);
		                    }
		                }
                	}
                }
     ...
     *  extend
     */
    private function moveToRemote($path,$fileext)
    {
        // mo ni login
        if($this->isForge == false)
        {
            if(is_uploaded_file($path))
            {
                ...
                $info = $this->fdfs->upload_filename($path,null,$fileext);    
                if ($info) {
                    //file md5
                   // $info['photo_id'] = md5_file($path);
                    if($this->isFileMd5){
                      $info['photo_id'] = md5_file($path);  
                    }
                    return $info;                    
